...
@section('content')
    ...
    <table class="table">
        ...
        <tbody>
            @foreach ($data as $aduans)
                <tr>
                    <th scope="row">{{ $loop->index+1 }}</th>
                    <td>{{ $aduans->pengadu->nama }}</td>
                    <td>{{ $aduans['nomor'] }}</td>
                    <td>{{ $aduans->jenisaduan->jenis_aduan }}</td>
                    <td>{{ $aduans['aduan'] }}</td>
                    <td>{{ $aduans['aduan_foto'] }}</td>
                    <td>
                        <form method="POST" action="{{ url('crud') }}/{{ $aduans['id'] }}">
                            @csrf
                            @method('DELETE')
                            <a href="{{ url('crud') }}/{{ $aduans['id'] }}/edit" type="button" class="btn btn-warning">jawab</a>
                        </form>
                    </td>
                </tr>
            @endforeach
